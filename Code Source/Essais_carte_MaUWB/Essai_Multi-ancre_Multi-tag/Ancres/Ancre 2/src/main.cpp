...
void setup() {
    ...
    sendATCommand("AT+SETCFG=2,1,0,1", 2000); // ID:2, Role:Ancre(1), Rate:850K(0), Filter:ON(1)
    ...
    updateScreen("ANCRE 2", "En attente du Tag...");
}
...
void loop() {
    if (UWBSerial.available()) {
        ...
        if (dataIndex != -1) {
            ...
            updateScreen("ANCRE 2 (Fixe)", distanceValue);
        }
    }
}
