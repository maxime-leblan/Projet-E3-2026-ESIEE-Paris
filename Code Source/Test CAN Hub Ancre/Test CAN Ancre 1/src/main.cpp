...
void loop() {
    ...
    Serial.println("On essaie d'envoyer un message à l'ancre 2 par CAN");
    sendCanDistanceFromAnchorToHub(MY_ANCHOR_ID, 4, {1, 2, 3, 4});

    delay(2000);

    // on vérifie si l'ancre 2 nous a répondu
    if (receiveCanMessage(vCanMessage))
    {
        if (decodeCanMessage(vCanMessage, vDecodedCanData))
        {
            Serial.println("Message reçu envoyé par l'ancre 2 : tag " + String(vDecodedCanData.id_tag) + ", distance " + String(vDecodedCanData.distance));
        }
        else
        {
            Serial.println("Erreur de décodage du message provenant de l'ancre 2");
        }
    }
